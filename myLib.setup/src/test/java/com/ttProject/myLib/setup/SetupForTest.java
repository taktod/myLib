...
public class SetupForTest {
	...
	private IVideoPicture image() {
		...
		IVideoPicture picture = converter.toPicture(base, 100000 * videoCounter);
		videoCounter ++;
		return picture;
	}
	@Test
	...
	public void aacSetup() throws Exception {
		...
		// flvデータを作ります。
		IContainer container = IContainer.make();
		if(container.open("test.aac", IContainer.Type.WRITE, null) < 0) {
			throw new Exception("開けませんでした");
		}
	...
	public void h264Setup() throws Exception {
		...
		// flvデータを作ります。
		IContainer container = IContainer.make();
		if(container.open("test264.flv", IContainer.Type.WRITE, null) < 0) {
			throw new Exception("開けませんでした");
		}
	...
	public void flvSetup() throws Exception {
		...
		while(counter < 20) {
			...
			while(true) {
				...
				//		long startPos = 1024 * audioCounter / 44100 * 1000;
				if(1024 * audioCounter / 44100 * 1000 > videoCounter * 100000) {
					break;
				}
			}
		}
