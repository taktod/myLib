 ...
 * @author taktod
 */
public class AdpcmPlayTest1D {
	/** ロガー */
	private Logger logger = Logger.getLogger(AdpcmPlayTest1D.class);
			...
			if(container instanceof MkvBlockTag) {
				...
				if(frame instanceof AdpcmImaWavFrame) {
					...
					while(buffer.remaining() > 0) {
						...
						completeBuffer.putShort((short)(second + (3) * (c0)));
						completeBuffer.putShort((short)(second + (4) * (c0)));
